import { CheckCircle2, CircleAlert, PlugZap, Radar, Smartphone, X } from 'lucide-react';
import { useState } from 'react';
import type { DeviceConfigurationDTO, DiscoveredDeviceDTO } from '../server/protocol';
...
const initialForm = (candidate: DiscoveredDeviceDTO): FormState => ({
  name: candidate.name,
  identifier: candidate.identifier,
  appId: candidate.platform === 'ANDROID' ? 'com.omnideck.market' : 'com.omnideck.market.ios',
  wdaBundleId: 'com.omnideck.WebDriverAgentRunner',
});
  ...
  return <div className="setup-backdrop" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) props.onClose(); }}>
    <aside className="device-setup-panel" aria-label="Device setup">
      <header className="setup-head">
        <div><span className="eyebrow">DEVICE ONBOARDING</span><h2>Detect & connect</h2><p>Configure one Android and one iPhone session.</p></div>
      ...
      <div className="setup-list">
        {(['ANDROID', 'IOS'] as const).map(platform => <section className="candidate-section" key={platform}>
          ...
          {props.candidates.filter(item => item.platform === platform).map(candidate => <CandidateCard key={candidate.candidateId} candidate={candidate} device={props.devices.find(item => item.id === candidate.deviceId)} onConfigure={props.onConfigure} onConnect={props.onConnect}/>) }
...
function CandidateCard({ candidate, device, onConfigure, onConnect }: { candidate: DiscoveredDeviceDTO; device?: Props['devices'][number]; onConfigure: Props['onConfigure']; onConnect: Props['onConnect'] }) {
  ...
  return <article className={`candidate-card ${candidate.platform.toLowerCase()}`}>
    ...
    <div className="candidate-actions">
      ...
      <button onClick={() => onConfigure(configuration)} disabled={!form.name || !form.identifier || !form.appId || (!dirty && Boolean(configured))}><CheckCircle2 size={14}/>{configured && !dirty ? 'Configured' : 'Save config'}</button>
