...
export class TaskScheduler {
  ...
  constructor(config: ConcurrencyConfig) { this.workers = new AgentWorkerPool(config); this.resources = new ResourceLimiter(config); this.rateLimiter = new RateLimiter(config.rateLimitPerMinute ?? 60); }
  ...
  createBatch(goal: string, deviceIds: string[], priority = 1): TaskInstance[] {
    ...
    return deviceIds.map((deviceId, index) => {
      const now = Date.now();
      const task: TaskInstance = { id: `${batchId}-${index + 1}`, batchId, deviceId, goal, status: 'WAITING', priority, attempts: 0, createdAt: now, updatedAt: now };
      return this.workers.enqueue(task);
    });
  }
}
